...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'plasma':
				dialogue = CoolUtil.coolTextFile(Paths.txt('plasma/plasmaDialogue'));
			case 'standing':
				dialogue = CoolUtil.coolTextFile(Paths.txt('standing/standingDialogue'));
			case 'gamer':
				dialogue = CoolUtil.coolTextFile(Paths.txt('gamer/gamerDialogue'));
			case 'megalojania':
				dialogue = CoolUtil.coolTextFile(Paths.txt('megalojania/sansDialogue'));
		}
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'sketch':
			{
					defaultCamZoom = 0.9;
					curStage = 'sketch';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('plasma/stageback'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);

					var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('plasma/stagefront'));
					...
					var stageCurtains:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('plasma/stagecurtains'));
	...
	function sans(dialogueBox:DialogueBox):Void
	{
		new FlxTimer().start(0.5,function(swagTimer:FlxTimer)
		{	
		camFollow.x += -600;
		camFollow.y += 0;
		FlxG.sound.play(Paths.sound('sanssound'), 1, false, null, true);
		FlxTween.tween(FlxG.camera, {zoom: 1.2},.25);
		dad.animation.play('sans', true);
		});
	}
	...
	*/
	...
	override function stepHit()
	{
		...
		if (dad.curCharacter == 'sansoj' && SONG.song.toLowerCase() == 'megalojania')
		{
			if (curStep == 32)
			{
